...
class ProsesPasien implements KontrakPresenter
{
	...
	function prosesDataPasien_byId($id)
	{
		try {
			//mengambil data di tabel pasien
			$this->tabelpasien->open();
			$this->tabelpasien->getPasien_byId($id);
			$this->data[] = $this->tabelpasien->getResult(); //tambahkan data pasien ke dalam list
			
		} catch (Exception $e) {
			//memproses error
			echo "gagal prosesDataPasien_byId pada ProsesPasien.php" . $e->getMessage();
		}
	}
	function tambahDataPasien($new_pasien)
	{
		//masukan data baru ke tabel pasien
		$this->tabelpasien->open();
		$this->tabelpasien->insertPasien($new_pasien);
		//tutup koneksi
		$this->tabelpasien->close();
	}
	function hapusDataPasien($id)
	{
		//hapus data berdasarkan id pada tabel pasien
		$this->tabelpasien->open();
	...
	{
		//update data pasien berdasarkan id nya
		$this->tabelpasien->open();
